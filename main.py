...
import credential
from tencentcloud.common.profile.client_profile
import ClientProfile
from tencentcloud.common.profile.http_profile
import HttpProfile
from tencentcloud.cdn.v20180606
import cdn_client, models
from astrbot.api.star
import register, Star, Context
...
logger.debug(f "zone_id 值前6位: {str(required_config['zone_id'])[:6]}")# 配置完整性校验
if not all(required_config.values()):
    missing = [k
        for k, v in required_config.items() if not v
    ]
logger.error(f "配置缺失关键参数: {', '.join(missing)}")
return False# 验证zone_id格式
...
if self._load_config(new_config):
    ...
    logger.error("配置重载失败")@ filter.command("cdn")
async def handle_cdn_command(self, event: AstrMessageEvent):
    ''
'CDN缓存刷新/预热（修正事件对象）'
''
try: #统一使用 event.message
if not event.message.startswith("/cdn"):
    return
...
yield event.plain_result(f "❌ 操作失败: {str(e)}")# 使用统一响应接口
...
""
try:
logger.debug(f "初始化SDK参数: region={region}, zone_id={zone_id}")# 凭证初始化
self.cred = credential.Credential(secret_id, secret_key)# HTTP客户端配置
